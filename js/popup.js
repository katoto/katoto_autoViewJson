$(document).ready(function() {
  ...
  getZone();
  function getZone() {
    bgZone = chrome.extension && chrome.extension.getBackgroundPage(); //获取background页面,popup与background通信
    console.log(bgZone);
    if (bgZone) {
  ...
  $("#outBtn").click(() => {
    // 得到最新变量
    if (bgZone) {
        // $("#outJson").val(JSON.stringify(bgZone.getkeyMd5Obj()));
        $("#outJson").val(JSON.stringify(bgZone.getInpStatic()));
        console.log('============')
      } else {
        Toast("插件视乎有点问题");
      }
  ...
  function sendBgInpJson(inpJson) {
    if (bgZone) {
      bgZone.setInpStatic (inpJson)
      bgZone.setkeyMd5Obj(changeKey2Md5(inpJson))
      console.log('============')
...
/*
 *@method changeKey2Md5
 * return json key keyMap
 */
function changeKey2Md5(obj = {}) {
  let deepFindKey = function(obj, baseStr = "") {
    let deepKey = [];
    function isObject(o) {
    ...
    Object.keys(obj).forEach(key => {
      if (isObject(obj[key])) {
        deepKey = deepKey.concat(deepFindKey(obj[key], `${baseStr}['${key}']`));
      } else {
        deepKey.push(`${baseStr}['${key}']`);
      }
    });
    return deepKey;
  };
  let newMd52 = {};
  deepFindKey(obj).forEach((item, index) => {
    newMd52[md5(item)] = item;
  });
  console.log(newMd52);
  return newMd52;
}
